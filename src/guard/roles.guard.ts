...
export class RolesGuard implements CanActivate {
  constructor(private reflector: Reflector, private role?: UserRole[]) { }
  ...
  canActivate(
    ...
    if (!hasRole) {
      throw new HttpException(`Access denied. Required roles: ${requiredRoles.join(', ')}, but user has role: ${user.role}`, HttpStatus.UNAUTHORIZED);
    }
